    ...
    do{
	    ...
	    printf(str);
            ...
            if (analyzer == 0) {
                ...
                //se è già stato avviato il processo di report devo insere il suo pid
                if (report != -1) {
                    analyzer_argv[arg_index++] = "-r";
                    char pid_report[8];
                    itoa(report, pid_report);
                    analyzer_argv[arg_index++] = pid_report;
                    printf("Analisi passata al report\n");
                }
