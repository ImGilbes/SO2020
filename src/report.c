...
int main(int argc, char **argv){
    ...
    while(!doneFlag){ //ricevo input finchè non riceve comando di terminazione
        ...
        if(strcmp(buff,"done") != 0){ //done è il comando di fine input
            ...
            fname = strtok(buff, delim);//leggo il file, se già è nella lista dei file ok, altrimenti devo creare una struttura file_analysis nuova per il file
            curFile = fileStructPointer(fileList, fname);
            charID = atoi(strtok(NULL,delim));
            charDATA = atoi(strtok(NULL,delim));
            curFile->analysis[charID] = charDATA;//aggiungo i dati sul carattere nella struttura apposita
    ...
    //per ogni file calcolo il totale dei caratteri
    ...
    while ((curFile = (struct file_analysis *)list_iterator_next(iter))){
        ...
        for(i = 0; i < 128; i++ ){
            if(curFile->analysis[i] > 0){ //stampo i dati su un caratteri sse è stato trovato almeno una volta
                printf("caratteri %c: %d (%f) \n", i, (int)curFile->analysis[i], ((float) curFile->analysis[i] / totChars[k]));
            }
        }
        k++;
    }
